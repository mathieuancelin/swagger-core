...
public class SwaggerPlugin extends PlayPlugin {
    ...
    private static final String RESOURCES_XML = play.Play.configuration.getProperty("swagger.api.resPath", "/resources") + XML;
    private static final String RESOURCES_JSON = play.Play.configuration.getProperty("swagger.api.resPath", "/resources") + JSON;
    private static final String PREFIX = play.Play.configuration.getProperty("swagger.api.prefix", "");
...
//        Logger.info("Got request for " + request.path);
        ...
        try {
            ...
            for (String resourceName : resourceNames) {
                if (request.path.equals(PREFIX + resourceName + JSON)) {
    ...
    public void onApplicationStart() {
        ...
        if (resourceNames.size() > 0) {
            Router.prependRoute("GET", PREFIX + RESOURCES_XML, "ApiHelpController.catchAll");
            Router.prependRoute("GET", PREFIX + RESOURCES_JSON, "ApiHelpController.catchAll");
            Logger.info("Swagger: Added ROOT help api @ " + PREFIX + RESOURCES_JSON);
            Logger.info("Swagger: Added ROOT help api @ " + PREFIX + RESOURCES_XML);
            for (String resourceName : resourceNames) {
                ...
                Logger.info("Swagger: Added help api @ " + PREFIX + resourceName + ".json");
                Logger.info("Swagger: Added help api @ " + PREFIX + resourceName + ".xml");
            }
        }
    }
